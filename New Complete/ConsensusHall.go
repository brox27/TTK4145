		...
		case remoteHallConsensus := <-hallordersRx:
			for floor := 0; floor < ConfigFile.Num_floors; floor++ {
				for button := 0; button < ConfigFile.Num_buttons-1; button++ {
					...
					merge(local, remote) // sette til å oppdatere local også!!!
				}
			}
		default:
			break
...
func merge(local ConfigFile.Orderstate, remote ConfigFile.Orderstate) {
	RemoteID = "123"
	LocalID = "321"
	...
	case ConfigFile.Active:
		...
		case ConfigFile.Active:
			// appendet IP til ACked by her også
			break
